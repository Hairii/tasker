...
// récupérer une tâche par son id (pour vérifier la propriété)
export const getTaskById = async (id) => {
    try {
        const [rows] = await db.query("SELECT * FROM task WHERE id = ?", [id]);
        return rows[0];
    } catch (error) {
        console.error("erreur getTaskById", error.message);
        throw error;
    }
};
...
export const updateTask = async (id, { status }) => {
    try {
        await db.query("UPDATE task SET status = ? WHERE id = ?", [status, id]);
    } catch (error) {
